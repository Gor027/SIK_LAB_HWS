...
ssize_t readContent(int s, char *line, size_t line_size,
...
uint64_t readSize(int s, char *line) {
    size_t line_size = 65;
    char *end_ptr;

    if (readContent(s, line, line_size, "read int64") == 0) {
        syserr("read size");
    }
...
void listener_socket_cb(evutil_socket_t sock, short ev, void *arg) {
    ...
    char buf[BUF_SIZE];
    /* Get file name size */
    ssize_t fileNameSize = readSize(connection_socket, buf);

    /* File Name Reading */
    if (readContent(connection_socket, buf, fileNameSize, "fileName reading") != fileNameSize) {
    ...
    /* File size reading */
    uint64_t fileSize = readSize(connection_socket, buf);
